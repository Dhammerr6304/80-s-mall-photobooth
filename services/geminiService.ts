...
const initialPrompt = "A classic mall photography style photoshoot of this person from the 80s with a cheesy background and 80s style clothing. Never respond with text. Just generate the photo.";
...
export const generateVideoFromImage = async (
    ...
    // Create a new instance to ensure the most up-to-date key is used
    const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
    ...
    const res = await fetch(`${video.video.uri}&key=${process.env.API_KEY}`);
    if (!res.ok) {
